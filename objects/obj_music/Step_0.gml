...
if (global.panic && global.leveltosave != "dragonlair")
{
	...
	else if (fmod_event_instance_is_playing(panicmusicID))
	{
		...
		{
			if global.laps >= 3 && instance_exists(obj_snickexe) // lap 4
		    fmod_event_instance_set_parameter(panicmusicID, "state", 8, true);
			else if global.laps > 1 && instance_exists(obj_pizzaface) // pillar john's revenge
