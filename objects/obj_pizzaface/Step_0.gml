...
with (obj_player)
{
	if (state == states.taxi || state == states.victory || state == states.keyget || state == states.gottreasure || state == states.door || state == states.spaceshuttle || state == states.fireass || state == states.bombpep)
		_move = false;
}
if (!treasure)
{
